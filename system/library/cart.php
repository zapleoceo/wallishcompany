...
class Cart {
	...
	public function getProducts($data = array()) {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				$product_data[] = array(
					...
					'recurring'       => $recurring,
                    'new' => $product_query->row['new'],
                    'sale' => $product_query->row['sale']
                );
			} else {
        ...
        $product_data = $this->load->controller('module/custmer_group_by_summ/productsPrices', $product_data);
	...
	public function getSubTotal($data = array(), $products = false) {
		$total = 0;
        $products = $products != false ? $products : $this->getProducts($data);

		foreach ($products as $product) {
		    if (isset($data['discount_exlude']) && $data['discount_exlude'] == 'sale' && $product['sale'])
		        continue;
