...
class ModelReportSale extends Model {
	...
	public function getTotalOrdersByCountry() {
		$query = $this->db->query("SELECT COUNT(*) AS total, SUM(o.total) AS amount, c.iso_code_2 FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "country` c ON (o.payment_country_id = c.country_id) WHERE o.order_status_id > '0' AND o.payment_country_id > '0' GROUP BY o.payment_country_id");
	...
	public function getCategoriesIdsByProductsIds($products_ids) {
	    if (empty($products_ids))
	        return false;

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id IN(" . implode(',', $products_ids) . ") AND main_category = 1");
	...
	public function getProductsIdsByOrdersIds($orders_ids) {
	    if (empty($orders_ids)) return false;

        $query = $this->db->query("SELECT op.*,ot.sale FROM `" . DB_PREFIX . "order_product` as op 
            LEFT JOIN `" . DB_PREFIX . "product_description` ot ON (op.product_id = ot.product_id)
                WHERE op.order_id IN(" . implode(",", $orders_ids) . ") GROUP BY op.order_product_id");
        ...
        $sql = "SELECT 
                    ...
                    SUM((SELECT SUM(ot.value) 
                        FROM `" . DB_PREFIX . "order_total` ot 
                            WHERE ot.order_id = o.order_id AND ot.code = 'tax' GROUP BY ot.order_id)) AS tax, 
                    SUM((SELECT SUM(ot.value) 
                        FROM `" . DB_PREFIX . "order_total` ot 
                            WHERE ot.order_id = o.order_id AND ot.code = 'sub_total' GROUP BY ot.order_id)) AS sub_total,
                    SUM((SELECT SUM(ot.value) 
                        FROM `" . DB_PREFIX . "order_total` ot 
                            WHERE ot.order_id = o.order_id AND ot.code = 'discount' GROUP BY ot.order_id)) AS discount,
