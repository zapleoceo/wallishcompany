...
class ModelCatalogAttribute extends Model {
	public function addAttribute($data) {
		...
		foreach ($data['attribute_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "attribute_description SET attribute_id = '" . (int)$attribute_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', value = '" . $this->db->escape($value['value']) . "'");
		}
	...
	public function getAttributeDescriptions($attribute_id) {
		...
		foreach ($query->rows as $result) {
			$attribute_data[$result['language_id']] = array('name' => $result['name'], 'value' => html_entity_decode($result['value'], ENT_QUOTES, 'UTF-8'));
		}
