...
class ModelCatalogProduct extends Model {
	...
	public function editProduct($product_id, $data) {
		...
		foreach ($data['product_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', description = '" . $this->db->escape($value['description']) . "', tag = '" . $this->db->escape($value['tag']) . "', meta_title = '" . $this->db->escape($value['meta_title']) . "', meta_h1 = '" . $this->db->escape($value['meta_h1']) . "', new = " . (int)$data['new'] . ", sale = " . (int)$data['sale'] . ", fete = " . (int)$data['fete'] . ", meta_description = '" . $this->db->escape($value['meta_description']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "'");
		}
    ...
    public function editProductNew($product_id, $status) {
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET new = '" . (int)$status . "' WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function editProductSale($product_id, $status) {
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET sale = '" . (int)$status . "' WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function editProductFete($product_id, $status) {
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET fete = '" . (int)$status . "' WHERE product_id = '" . (int)$product_id . "'");
    ...
    public function getProductMainCategoryIds($products_ids) {
        $query = $this->db->query("SELECT category_id,product_id FROM " . DB_PREFIX . "product_to_category WHERE product_id IN (" . implode(',', $products_ids) . ") AND main_category = '1' LIMIT 1");
