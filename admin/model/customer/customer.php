...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
                $old_address = $this->getAddress((int) $address['address_id']);
                ...
                if (!empty($old_address))
                    $address['custom_field'] = array_merge($old_address['custom_field'], $address['custom_field']);
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
                ...
                if (!empty($address['address_id'])) {
                    $this->db->query("UPDATE " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : '') . "' WHERE address_id = '" . (int)$address['address_id'] . "'");
                } else {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id . "', firstname = '" . $this->db->escape($address['firstname']) . "', lastname = '" . $this->db->escape($address['lastname']) . "', company = '" . $this->db->escape($address['company']) . "', address_1 = '" . $this->db->escape($address['address_1']) . "', address_2 = '" . $this->db->escape($address['address_2']) . "', city = '" . $this->db->escape($address['city']) . "', postcode = '" . $this->db->escape($address['postcode']) . "', country_id = '" . (int)$address['country_id'] . "', zone_id = '', custom_field = '" . $this->db->escape(isset($address['custom_field']) ? json_encode($address['custom_field']) : '') . "'");
                }
	...
	public function getCustomers($data = array(), $issend = false) {
		...
		if ($issend) {
            ...
            if (!empty($query2)) {
                ...
                $query->rows = array_merge($query->rows, $query2->rows);
            }
        }
	...
	public function getAddresses($customer_id) {
		...
		foreach ($query->rows as $result) {
            ...
            if (isset($address_info['custom_field']['city_id']) && !empty($address_info['custom_field']['city_id']))
                $address_info['np_cities'] = $this->getCities($address_info['custom_field']['region_id']);

            if (isset($address_info['custom_field']['address_id']) && !empty($address_info['custom_field']['address_id']))
                $address_info['np_addresses'] = $this->getNPAdress($address_info['custom_field']['city_id']);
	...
	public function getTotalCustomers($data = array(), $is_send = false) {
		...
		if ($is_send) {
            $sql2 = "SELECT COUNT(*) AS total FROM subscribers";
            $query2 = $this->db->query($sql2);
            if (!empty($query2->row['total'])) {
                $query->row['total'] = $query->row['total'] + $query2->row['total'];
            }
        }
