...
class ModelLocalisationCountry extends Model {
    ...
    public function editCountry($country_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "countries SET name = '" . $this->db->escape($data['name']) . "' WHERE id = '" . (int)$country_id . "'");
	...
	public function getCountry($country_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "country WHERE country_id = '" . (int)$country_id . "' ORDER BY name ASC");

		return $query->row;
	}

    public function getCountryInfo($id) {
        $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "countries WHERE id = '" . (int)$id . "'");
	...
	public function getCountries($data = array()) {
		if ($data) {
			...
			$country_data = $this->cache->get('countries');

			if (!$country_data) {
				$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "countries ORDER BY name ASC");

				$country_data = $query->rows;

				$this->cache->set('countries', $country_data);
			}
