...
class ControllerModuleWeMenu extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            $this->request->post['we_menu'] = $this->prepare($this->request->post['we_menu']);
			...
			$this->response->redirect($this->url->link('module/we_menu', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL'));
		}
        ...
        $data['we_menu'] = $this->prepare($data['we_menu']);
        ...
        foreach($menus as &$menu) {
            ...
            if (empty($names)) {
                foreach($languages as  $lang) {
                    $names[$lang['language_id']] = $menu['item'];
                }
            }
            ...
            foreach ($this->request->post['we_menu'] as $i => $item) {
                if ($item['item_type'] == 'clear_link') {
                    ...
                    foreach($languages as $l) {
                        if(isset($item['item'][$l['language_id']])) {
                            $newitem[$l['language_id']] = $item['item'][$l['language_id']];
                        } else {
                            $newitem[$l['language_id']] = '';
                        }
                    }
