...
class ControllerCustomerCustomer extends Controller {
    ...
    private function load_model($model) {
        require(DIR_CATALOG.'/model/'.$model.'.php');
    ...
    public function getcities() {
        ...
        $cities = $this->model_customer_customer->getCities($country_id);
        if (empty($cities))
    ...
    public function getnpadress() {
        ...
        $adreses_np = $this->model_customer_customer->getNPAdress($zone_id);
        if (empty($adreses_np))
            exit(json_encode(array('ok' => false)));
	...
	protected function getForm() {
        ...
        $data['entry_address_type'] = $this->language->get('entry_address_type');
        $data['entry_shipping_method'] = $this->language->get('entry_shipping_method');
        $data['entry_zone_option'] = $this->language->get('entry_zone_option');
        $data['entry_zone_select'] = $this->language->get('entry_zone_select');
        $data['entry_city_option'] = $this->language->get('entry_city_option');
        $data['entry_city_select'] = $this->language->get('entry_city_select');
        $data['entry_npselect_option'] = $this->language->get('entry_npselect_option');
        $data['entry_npselect'] = $this->language->get('entry_npselect');

        // Select
        $data['select_address_type1'] = $this->language->get('select_address_type1');
        $data['select_address_type2'] = $this->language->get('select_address_type2');
        ...
        $this->load_model('extension/extension');

        $results = $this->model_extension_extension->getExtensions('shipping');

        foreach ($results as $result) {
            if ($this->config->get($result['code'] . '_status')) {
                $this->load_model('shipping/' . $result['code']);
        ...
        $this->load_model('account/address');
        ...
        $data['our_address'] = json_decode($address, true);
