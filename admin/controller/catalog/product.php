...
class ControllerCatalogProduct extends Controller {
	...
	public function chsale() {
		...
		$status = (int)$this->request->get['st'];

		if (empty($product_id))
			die();
		
		$this->model_catalog_product->editProductSale($product_id, $status);
	}

	public function chnew() {
		...
		$this->model_catalog_product->editProductNew($product_id, $status);
	}

    public function chfete() {
        ...
        $this->model_catalog_product->editProductFete($product_id, $status);
    }

	public function chpricesale() {
        $this->load->model('catalog/product');

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category p2c WHERE category_id = '59' ORDER BY p2c.product_id");
        ...
        //var_dump($prod_string);
        //$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product p ORDER BY p.product_id");
        ...
        foreach ($query->rows as $product) {
            //$this->db->query("UPDATE " . DB_PREFIX . "product_description SET sale = '0' WHERE product_id = '" . $product['product_id'] . "'");
            //$this->db->query("DELETE FROM " . DB_PREFIX . "product_special WHERE product_id = '" . $product['product_id'] . "'");

            $new_price = round(($product['price']*0.9));
            //$this->db->query("UPDATE " . DB_PREFIX . "product SET price = '" . $new_price . "' WHERE product_id = '" . $product['product_id'] . "'");
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            /*$range_price = $this->request->post['range_price'];
            if (!empty($range_price)) {

                $dataupdate = array();
                foreach($range_price as $index => $rprice) {
                    $rprice = (int)$rprice;

                    $path = 'cpprods_'.$product_id. '_' . $index.'_value';
                    $dataupdate[$path] = number_format($rprice, 2, '.', '');
                }

                $this->load->model('setting/setting');

                $this->model_setting_setting->editSetting('cpprods_' . $product_id, $dataupdate);
            }*/
	...
	protected function getForm() {
        ...
        if (isset($this->request->post['fete'])) {
            $data['fete'] = $this->request->post['fete'];
        } elseif (!empty($product_info)) {
            $data['fete'] = $product_info['fete'];
        } else {
            $data['fete'] = 0;
        }
        ...
        // prices Range
        /*$params = isset($this->request->get['product_id']) ? array('product_id' => $this->request->get['product_id']) : array();
        $data['range_prices'] = $this->load->controller('module/custmer_group_by_summ/getRanges', $params);*/
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['model']) < 1) || (utf8_strlen($this->request->post['model']) > 64)) {
            ...
            preg_match('/^[\w-]+-(\d+)$/', $this->request->post['model'], $pmodel_arr);

            if (is_array($pmodel_arr) && !empty($pmodel_arr[1])) {
                $this->request->post['sort_order'] = $pmodel_arr[1];
            }
        }
