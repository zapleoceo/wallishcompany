...
class ControllerSaleOrder extends Controller {
    ...
    protected function getCustomtList() {
        ...
        $this->response->setOutput($this->load->view('sale/order_list_custom.tpl', $data));
    }
		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				$data['products'][] = array(
					...
					'name'    	 	   => $product['name'],
					'model'    		   => $product['model'],
					'image'            => array('image' => $image, 'thumb' => $this->model_tool_image->resize($thumb, 500, 500)),
					'option'   		   => $option_data,
					'quantity'		   => $product['quantity'],
	...
	public function invoice() {
        ...
        $data['ispdf'] = isset($this->request->get['pdf']) ? 1 : 0;

        if ($data['ispdf']) {
            if (isset($this->request->post['image']) && !empty($this->request->post['image'])) {
                $html = array();
                ...
                //print_r(implode('<br>', $html));exit();

                $mpdf = new \Mpdf\Mpdf();
                $mpdf->WriteHTML(implode('<br><br>', $html));
                $mpdf->Output('report_' . time() . '.pdf', 'D');
                exit();
            }

            $data['scripts'] = array(STYLE_PATH . 'js/html2canvas.js');
        }
			...
			if ($order_info) {
				...
				foreach ($products as $product) {
					...
					$product_data[] = array(
						...
						'option'   => $option_data,
						'quantity' => $product['quantity'],
                        'image'    => array('image' => $image, 'thumb' => $this->model_tool_image->resize($thumb, 70, 70)),
                        'price'    => $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']),
						'total'    => $this->currency->format($product['total'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value'])
					);
				}
    ...
    public function changeStatus() {
        ...
        if (!$this->user->hasPermission('modify', 'sale/order')) {
            $json['error'] = $this->language->get('error_permission');
        } elseif (isset($this->request->get['order_id'], $this->request->post['status_id'])) {
            $order_id = $this->request->get['order_id'];
            $status_id = $this->request->post['status_id'];

            $this->load->model('sale/order');

            $update_status = $this->model_sale_order->updateStatus($order_id, $status_id);

            if ($update_status) {
                $json['success'] = 'Order status updated!';
            } else {
                $json['error'] = $this->language->get('error_action');
            }
        }
