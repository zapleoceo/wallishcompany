...
class ControllerCommonMenu extends Controller {
	public function index() {
		...
		$data['text_pricer'] = $this->language->get('text_pricer');
		...
		$data['pricer'] = $this->url->link('catalog/pricer', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $links = $this->url->get_links();

        $skips = array('home');
        foreach($data as $lkey => $val) {
            if (in_array($lkey, $skips))
                continue;
            ...
            $route = $links[$val];
            //var_dump((int)$this->user->hasPermission('modify', $route) . ' - ' .$route);
