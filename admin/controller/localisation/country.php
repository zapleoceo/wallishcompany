...
class ControllerLocalisationCountry extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['countries'][] = array(
				...
				'name'       => $result['name'] . (($result['id'] == $this->config->get('config_country_id')) ? $this->language->get('text_default') : null),
				'iso_code_2' => '',
				'iso_code_3' => '',
				'edit'       => $this->url->link('localisation/country/edit', 'token=' . $this->session->data['token'] . '&country_id=' . $result['id'] . $url, 'SSL')
			);
		}
	...
	protected function getForm() {
		...
		if (isset($this->request->get['country_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$country_info = $this->model_localisation_country->getCountryInfo($this->request->get['country_id']);
		}
