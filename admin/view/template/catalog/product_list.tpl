              ...
              <tbody>
                ...
                <tr>
                  <td class="text-center"><?php if (in_array($product['product_id'], $selected)) { ?>
                    ...
                    <td class="text-center"><?php if ($product['new']) { ?>
                    <input type="checkbox" name="selected_new[]" value="<?php echo $product['product_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected_new[]" value="<?php echo $product['product_id']; ?>" />
                    <?php } ?></td>
                    
                    <td class="text-center"><?php if ($product['sale']) { ?>
                    <input type="checkbox" name="selected_sale[]" value="<?php echo $product['product_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected_sale[]" value="<?php echo $product['product_id']; ?>" />
                    <?php } ?></td>

                    <td class="text-center"><?php if ($product['fete']) { ?>
                    <input type="checkbox" name="selected_fete[]" value="<?php echo $product['product_id']; ?>" checked="checked" />
                    <?php } else { ?>
                    <input type="checkbox" name="selected_fete[]" value="<?php echo $product['product_id']; ?>" />
                    <?php } ?></td>
...
$(document).ready(function(){
  ...
  $('input[name*=\'selected_new\']').on('change', function(){
      if ($(this).is(':checked')) {
        ...
        $.ajax({
          url: 'index.php?route=catalog/product/chnew&token=<?php echo $token; ?>&product_id=' + $(this).val() + '&st=0',
        });      
      }
  });

  $('input[name*=\'selected_fete\']').on('change', function () {
    if ($(this).is(':checked')) {
      $.ajax({
        url: 'index.php?route=catalog/product/chfete&token=<?php echo $token; ?>&product_id=' + $(this).val() + '&st=1',
      });
    } else {
      $.ajax({
        url: 'index.php?route=catalog/product/chfete&token=<?php echo $token; ?>&product_id=' + $(this).val() + '&st=0',
      });
    }
  });
