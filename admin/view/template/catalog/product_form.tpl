    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          ...
          <input type="hidden" value="<?= $fete; ?>" name="fete">
          ...
          <div class="tab-content">
            ...
            <div class="tab-pane" id="tab-data">
              ...
              <?php if (!empty($range_prices)): ?>
                <hr>
                <label>Цены: </label>
                <hr>

                <?php foreach($range_prices as $rprice): ?>
                <div class="form-group">
                  <label class="col-sm-2 control-label" for="range-price-<?= $rprice['index']; ?>">
                    <?= $rprice['min']; ?>-<?= $rprice['max']; ?> грн.
                  </label>
                  <div class="col-sm-10">
                    <input type="text" name="range_price[<?= $rprice['index']; ?>]" value="<?= $rprice['value']; ?>" placeholder="" id="range-price-<?= $rprice['index']; ?>" class="form-control" />
                  </div>
                </div>
                <?php endforeach; ?>
              <?php endif; ?>
...
function attributeautocomplete(attribute_row) {
	$('input[name=\'product_attribute[' + attribute_row + '][name]\']').autocomplete({
		...
		'select': function(item) {
		    if (typeof item.values !== 'undefinded') {
                $.each(item.values, function(lang_id, val){
                    $('textarea[name=\'product_attribute[' + attribute_row + '][product_attribute_description][' + lang_id + '][text]\']').val(val.value);
                });
            }
