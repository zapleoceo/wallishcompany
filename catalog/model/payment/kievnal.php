...
class ModelpaymentKievnal extends Model {
	...
	public function getMethod($address, $total) {
		...
		if ($this->config->get($this->code . '_total') > 0 && $this->config->get($this->code . '_total') > $total) {
			$status = false;
		} elseif (!$this->config->get($this->code . '_geo_zone_id')) {
			$status = true;
