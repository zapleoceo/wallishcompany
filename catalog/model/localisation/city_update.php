...
class ModelLocalisationCity extends Model {
    ...
    public function getCities($data = array()) {
		...
		if (!$city_data) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "city WHERE status = '1' ORDER BY code ASC");
    ...
    public function getCitiesByZoneId($zone_id) {
		...
		if (!$city_data) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "city WHERE zone_id = '" . (int)$zone_id . "' AND status = '1' ORDER BY code ASC");
