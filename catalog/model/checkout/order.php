...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		// Логируем данные заказа перед созданием
		$log_file = DIR_LOGS . 'order_creation.log';
		$timestamp = date('Y-m-d H:i:s');
		$log_entry = "[{$timestamp}] ORDER_ADD_START | ORDER_DATA: " . json_encode($data, JSON_UNESCAPED_UNICODE) . "\n";
		...
		// Логируем созданный заказ
		$log_file = DIR_LOGS . 'order_creation.log';
		$timestamp = date('Y-m-d H:i:s');
		$log_entry = "[{$timestamp}] ORDER_CREATED | ORDER_ID: {$order_id} | CUSTOMER_DATA: " . json_encode(array(
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
			    if (!isset($product['saleorder']))
                    $product['saleorder'] = number_format(0, 2, '.', '');

			    $this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "' , saleorder = " . (int)$product['saleorder'] . "");
        ...
        $sales_data = array();
		/*
		if (isset($data['products']) && $discount) {
		    $pids = array_column($data['products'], 'product_id');

		    if ($pids) {
                $sales = $this->db->query("SELECT sale,product_id FROM `" . DB_PREFIX . "product_description` WHERE product_id IN(" . implode(',', $pids) . ") GROUP BY product_id");
                foreach ($sales->rows as $sa) {
                    $sales_data[$sa['product_id']] = $sa['sale'];
                }
            }
            ...
            foreach($data['products'] as $pro) {
                $tosale = isset($sales_data[$pro['product_id']]) ? $sales_data[$pro['product_id']]: 0;
            ...
            $tosale2 = $totalsale + $discount;
            $persent_sale = 100-$tosale2/$totalsale*100;

            foreach($data['products'] as $pro) {
                $tosale = isset($sales_data[$pro['product_id']]) ? $sales_data[$pro['product_id']]: 0;

                if ($tosale == 0 && $persent_sale) {


                    $salesave = 0;

                    $salesave = (int)$totalsale - ((int)$totalsale / 100 * $persent_sale);
                    $salesave = (int)$totalsale - $salesave;

                    $this->db->query("UPDATE " . DB_PREFIX . "order_product SET salethis = " . $salesave . " WHERE order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$pro['product_id'] . "'");
