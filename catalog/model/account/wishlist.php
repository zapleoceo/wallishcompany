...
class ModelAccountWishlist extends Model {
    ...
    public function delWishlist($product_id) {
        $this->event->trigger('pre.wishlist.del');
        ...
        $this->event->trigger('post.wishlist.del');
    }
	...
	public function getWishlist($offset=false, $limit=false) {
	    $limitoffset = array();
	    if ($offset !== false) {
	        $limitoffset[] = $offset;
        }
        ...
        $limitoffset = !empty($limitoffset) ? ' LIMIT ' . implode(',', $limitoffset) : '';

        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "customer_wishlist WHERE customer_id = '" . (int)$this->customer->getId() . "'" . $limitoffset);
