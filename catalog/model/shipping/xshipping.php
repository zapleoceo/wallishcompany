	...
	function getQuote($address) {
	    ...
	    $quote_data = array();
      		...
      		for($i=1;$i<=12;$i++)
			{
				...
				if (isset($this->cart) && $this->cart->getSubTotal() >= $free_shipping_cost && $free_shipping_cost!=0) {
					$shipping_cost = 0;
				...
				if ($status) {
					...
					$quote_data['xshipping'.$i] = array(
						...
						'text'         => isset($this->tax) ? $this->currency->format($this->tax->calculate($shipping_cost, $this->config->get('xshipping_tax_class_id'.$i), $this->config->get('config_tax'))) : ''
					);
