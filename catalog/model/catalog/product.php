...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
				...
				if (!empty($data['filter_skip_disable'])) {
        ...
        if (!empty($data['new']) && !empty($data['sale']) && !empty($data['fete'])) {
            $sql .= " AND (pd.sale = 1 OR pd.new = 1 OR pd.fete = 1)";
        } else {
            if (!empty($data['sale'])) {
                $sql .= " AND pd.sale = 1";
            }
		...
		if (!empty($data['filter_category_id'])) {
			if (!empty($data['filter_sub_category'])) {
                ...
                if (!empty($data['filter_skip_disable'])) {
                    $sql .= " AND c.status = 1  AND p2c.main_category = 1";
                }
			} else {
        ...
        //$product_data = $this->load->controller('module/custmer_group_by_summ/getPrices', $product_data);
	...
	public function getProductRelated($product_id) {
		...
		$query = $this->db->query("SELECT related_id FROM " . DB_PREFIX . "product_related pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
	...
	public function getProductWishlistIds($user_id = 0) {
	    if (empty($user_id))
	    ...
	    $query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "customer_wishlist WHERE customer_id='".$user_id."'");
		if (empty($query->rows))
			return array();
	...
	public function getProductNav($product_id, $category_id = false) {
		if (empty($product_id)) 
			return false;

        $sql = "SELECT sort_order FROM " . DB_PREFIX . "product WHERE product_id = '" . $product_id . "' LIMIT 1 ";
        $product = $this->db->query($sql);

        $sql = "SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '".$product_id."' AND main_category = 0 ORDER BY category_id DESC LIMIT 1 ";
        $sql = "SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '".$product_id."' ORDER BY category_id DESC LIMIT 1 ";

        $product_category = $this->db->query($sql);
        if (empty($product_category->num_rows)) {
            $sql = "SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $product_id . "' ORDER BY category_id DESC LIMIT 1 ";
            $product_category = $this->db->query($sql);
        }
//echo '<pre>'; var_dump($product_category);
/*
        if ($category_id) {
            $product_category->row['category_id'] = $category_id;
        }
*/
        if (empty($product->num_rows) || empty($product_category->num_rows)) return false;

        // предыдущий товар
        $sql = "SELECT " . DB_PREFIX . "product.product_id FROM " . DB_PREFIX . "product, " . DB_PREFIX . "product_to_category
                WHERE " . DB_PREFIX . "product.product_id = " . DB_PREFIX . "product_to_category.product_id
                AND " . DB_PREFIX . "product.sort_order >= ".$product->row['sort_order']."
                AND " . DB_PREFIX . "product.product_id != '".$product_id."'
                AND " . DB_PREFIX . "product_to_category.category_id = " . $product_category->row['category_id'] . "
                AND " . DB_PREFIX . "product.status = 1
                ORDER BY " . DB_PREFIX . "product.sort_order ASC LIMIT 1 ";
        $prev = $this->db->query($sql);

        if (empty($prev->num_rows)) {
            $sql = "SELECT " . DB_PREFIX . "product.product_id, MIN(" . DB_PREFIX . "product.sort_order)
                    FROM " . DB_PREFIX . "product, " . DB_PREFIX . "product_to_category
                    WHERE " . DB_PREFIX . "product.product_id = " . DB_PREFIX . "product_to_category.product_id
                          AND " . DB_PREFIX . "product_to_category.category_id = " . $product_category->row['category_id'] . "
                          AND " . DB_PREFIX . "product.status = 1
                          ORDER BY " . DB_PREFIX . "product.sort_order ASC
                          LIMIT 1";
        ...
        AND " . DB_PREFIX . "product_to_category.category_id = " . $product_category->row['category_id'] . "
        AND " . DB_PREFIX . "product.status = 1
        ORDER BY " . DB_PREFIX . "product.sort_order DESC LIMIT 1 ";
        $next = $this->db->query($sql);
//var_dump($product_id);
//echo '<pre>'; var_dump($product_id, $next); die;
        if (empty($next->num_rows)) {
            $next = $this->db->query("SELECT " . DB_PREFIX . "product.product_id FROM " . DB_PREFIX . "product, " . DB_PREFIX . "product_to_category
			WHERE " . DB_PREFIX . "product.product_id = " . DB_PREFIX . "product_to_category.product_id
			AND " . DB_PREFIX . "product.product_id != '" . $product_id . "'
			AND " . DB_PREFIX . "product_to_category.category_id = " . $product_category->row['category_id'] . "
			AND " . DB_PREFIX . "product.status = 1
		...
		return array(
			'prev' => $this->url->link('product/product', 'product_id=' . $prev->row['product_id']),
			//'next' => $this->url->link('product/product', 'product_id=' . $next->row['product_id']),
			'next' => $this->url->link('product/product', 'product_id=' . (isset($next->row['product_id']) ? $next->row['product_id'] : $product_id)),
		);
	}
...
/*
		if (empty($category_id) || empty($category_id->row))
			return array();
*/
	...
	public function getCategories($product_id, $ismain = false) {
	...
	public function getTotalProducts($data = array()) {
        ...
        if (!empty($data['filter_sale']) && !empty($data['filter_new']) && !empty($data['filter_fete'])) {
            $sql .= " AND (pd.sale = 1 OR pd.new = 1 OR pd.fete = 1)";
        } else {
