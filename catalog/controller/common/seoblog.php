...
if (!class_exists('ControllerCommonSeoBlog', false)) {
	class ControllerCommonSeoBlog extends Controller {
		...
		public function switchLanguage($language_id, $code) {
			...
			//Language switch for SEO multilang, if enabled
			if (count($this->languages_all) > 1 && isset($this->data['settings_general']['langmark_widget_status']) && $this->data['settings_general']['langmark_widget_status']) {
				return;
			}
			...
			if (!$this->registry->get('request_blog_router')) {
				...
				if ($this->flag_router == 'search' || $this->flag_router == 'record' || $this->flag_router == 'blog') {
					if (!$this->registry->get('fblogwork')) {
						...
						if (SC_VERSION > 15) {
							if (isset($this->journal2) || isset($this->journal3)) {
								new Action('record/' . $this->flag_router);
							}
