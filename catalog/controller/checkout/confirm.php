...
class ControllerCheckoutConfirm extends Controller {
	...
	public function index($param = false) {
		...
		if (!$redirect) {
            ...
            $order_data['totals'] = $this->load->controller('module/custmer_group_by_summ/productsAddSale', $order_data['totals']);
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$order_data['products'][] = array(
					...
					'price'      => $product['price'],
					'total'      => $product['total'],
					'pricelevel' => isset($product['pricelevel']) ? $product['pricelevel'] : null,
					'tax'        => $this->tax->getTax($product['price'], $product['tax_class_id']),
					'reward'     => $product['reward'],
                    'saleorder' => isset($product['saleorder']) ? $product['saleorder'] : null
				);
			}
			...
			$order_data['total'] = $this->load->controller('module/custmer_group_by_summ/getTotal', $order_data['totals']);;
            ...
            //exit(json_encode(array('ok' => true, 'html' => $payment, 'order_id' => $order_id)));
		} else {
            $this->response->redirect($redirect);
			//$data['redirect'] = $redirect;
            //exit(json_encode(array('ok' => true, 'redirect' => $redirect)));
		}

        //$this->load->controller('payment/' . $this->session->data['payment_method']['code'] . '/confirm');

        //$this->response->redirect($this->url->link('checkout/success', '&order_id=' . $order_id, 'SSL'));

        //$this->response->redirect($this->url->link('checkout/confirm/result', '&order_id=' . $order_id, 'SSL'));
    ...
    public function result() {
        ...
        $data['link_home'] = $this->url->link('common/home', '', 'SSL');
