...
class ControllerBlogArticle extends Controller {
	...
	public function get($setting) {
		if (empty($setting['article_id']))
			return false;

		$this->load->model('blog/article');
		$article_info = $this->model_blog_article->getArticle($setting['article_id']);
        $article_info['url'] = $this->url->link('blog/article', '' . '&article_id=' . $setting['article_id']);
        return $article_info;
    }

    public function getcategory($setting) {
	    if (empty($setting['category_id']))
	        return '';

	    $this->language->load('blog/topnews');
		$this->load->model('blog/article');

		$filt = array('filter_category_id' => $setting['category_id']);
		if (isset($setting['skips']) && !empty($setting['skips'])) {
		    $filt['skips'] = $setting['skips'];
        }

        if (isset($setting['limit']) && !empty($setting['limit'])) {
            $filt['limit'] = $setting['limit'];
        }

        if (isset($setting['start']) && !empty($setting['start'])) {
            $filt['start'] = $setting['start'];
        } else {
            $filt['start'] = 0;
        }

        if (isset($setting['rand']) && !empty($setting['rand'])) {
            $filt['rand'] = $setting['rand'];
        }

    $data['newsroom'] = $this->model_blog_article->getArticles($filt);
		foreach($data['newsroom'] as $key => &$article) {
      $article['url'] = $this->url->link('blog/article', '' . '&article_id=' . $article['article_id']);
		}


		$data['text_newsroom_title']  = $this->language->get('text_newsroom_title');
		$data['text_newsroom_more']  = $this->language->get('text_newsroom_more');
		$data['text_view_other_big']  = $this->language->get('text_view_other_big');

		return $this->load->view('default/template/blog/topnews.tpl', $data);
    }
	...
	if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
		...
		if ($article_info) {
            ...
            $this->document->addScript(STYLE_PATH . 'js/bootstrap.bundle.min.js' . CSSJS);
            $this->document->addScript(STYLE_PATH . 'js/owl.carousel.min.js' . CSSJS);
            $this->document->addScript(STYLE_PATH . 'js/newsroom.js' . CSSJS);
            ...
            $data['newsroom'] = $this->load->controller('blog/article/getcategory', array('category_id' => 73, 'skips' => [$article_id], 'rand' => true, 'limit' => 5));
